...
namespace UnitTesting02.Projects.Tests
{
    ...
    {
        [Fact]
        public void Constructor_WithIssueDiscriotionNull_ThrowInvalidIssueDescriptionException()
        {
            // Arrange
            // Act
        ...
        [Fact]
        public void Constructor_WithIssueDiscriotionIsWhiteSpace_ThrowInvalidIssueDescriptionException()
        {
            // Arrange
            // Act
        ...
        [Fact]
        public void Constructor_WithIssueNotProvidingCreatedAt_ReturnsCreatedAtEqualCurrentDateTime()
        {
            // Arrange
            var systemUnderTest = new Issue("Issue #1", Priority.Low, Category.Hardware);

            // Act
            var actual = systemUnderTest.CreatedAt;
        ...
        [Fact]
        public void Constructor_WithIssueValidProperities_Returns18CharIssueKey()
        {
            // Arrange
            var systemUnderTest = new Issue("issue #1", Priority.Low, Category.Hardware,
            ...
            var actual = methodInfo?.Invoke(systemUnderTest, null)?.ToString();
        ...
        public void GenerateKey_WithValidIssueProperties_ReturnsExpectedKey(string descritpion, 
            Priority priority, Category category, string createdAt, string expected)
        {
            // Arrange
            var systemUnderTest = new Issue(descritpion, priority, category, DateTime.Parse(createdAt));

            // Act
            MethodInfo methodInfo = typeof(Issue).GetMethod("GenerateKey", 
                BindingFlags.NonPublic | BindingFlags.Instance);

            var actual = methodInfo.Invoke(systemUnderTest, null);

            // Assert
            Assert.Equal(expected.Substring(0, 10), actual.ToString().Substring(0, 10));
        }
    }
}